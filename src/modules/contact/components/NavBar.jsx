    ...
    <header className="absolute top-0 left-0 w-full bg-transparent font-dm-sans tracking-wide z-50 px-4 sm:px-6 md:px-10 lg:px-20">
      <div className="w-full flex items-center justify-between h-[80px] py-2">
        ...
        <nav className="hidden lg:flex flex-wrap gap-6 md:gap-8">
          <NavLink to="/home" className={({ isActive }) => `text-white text-[14px] sm:text-[16px] border-b border-transparent hover:border-white transition duration-300 ${isActive ? 'border-b border-white' : ''}`}>Inicio</NavLink>
          ...
          <NavLink to="/" className={({ isActive }) => `text-white text-[14px] sm:text-[16px] border-b border-transparent hover:border-white transition duration-300 ${isActive ? 'border-b border-white' : ''}`}>Contacto</NavLink>
        </nav>
      ...
      <section className={`lg:hidden fixed top-0 left-0 w-full h-full bg-black/90 flex flex-col items-center justify-center transition-transform ${menuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <NavLink to="/home" className="py-3 text-white text-xl" onClick={() => setMenuOpen(false)}>Inicio</NavLink>
        ...
        <NavLink to="/" className="py-3 text-white text-xl" onClick={() => setMenuOpen(false)}>Contacto</NavLink>
        <NavLink 
          to="/auth/login" 
